...
def get_loss(model, images, texts, labels_image, labels_text, eos_indices, class_ids, args, epoch=-1):
    ...
    if args.aggregate:  
        ...
        # logits_text2text = logit_scale * text_features @ text_features.t()
...
def train(model, data, epoch, optimizer, scaler, scheduler, args, global_trained_steps):
    ...
    for i in range(global_trained_steps - num_batches_per_epoch * epoch, num_batches_per_epoch):
        ...
        images, texts, labels_image, labels_text, eos_indices, class_ids = batch
        ...
        if args.precision == "amp":
            with autocast():
                total_loss, acc = get_loss(model, images, texts, labels_image, labels_text, eos_indices, class_ids,
                                           args, epoch + 1)
            ...
            total_loss, acc = get_loss(model, images, texts, labels_image, labels_text, eos_indices, class_ids, args)
            total_loss.backward()
            optimizer.step()
        ...
        if is_master(args) and ((step + 1) % args.log_interval) == 0:
            ...
            logging.info(
                ...
                f"logit_scale: {m.logit_scale.data.exp():.3f} | " +
...
def evaluate(model, data, epoch, args, steps):
    ...
    if args.vision_model in ['RN50']:
        epoch_main, auc, map, loss_cls = eval_multiLabelCls_RN50(model, epoch + 1,
                                                                 cuda_rank=args.local_device_rank)
    else:
        epoch_main, auc, map, loss_cls = eval_multiLabelCls_ViT(model, epoch + 1,
                                                                cuda_rank=args.local_device_rank)
